      ...
      try
      {
        ...
        patientData = (from DataRow dr in dt.Rows
                       select new PatientModel()
                       {
                         ...
                         FamilyHistory = dr.IsNull("Family_History") ? string.Empty : Convert.ToString(dr["Family_History"]),
                         TotalReadmission = dr.IsNull("TotalReadmission") ? 0 : Convert.ToInt32(dr["TotalReadmission"])
                       }).ToList();
